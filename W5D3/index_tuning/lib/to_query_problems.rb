...
def no_apples_for_blair
  ...
  execute(<<-SQL)
    ...
    FROM
      cats JOIN
      cattoys ON cats.id = cattoys.cat_id JOIN
      toys ON toys.id = cattoys.toy_id
    WHERE 
...
def no_apples_for_blair_sub
  ...
  WHERE
    cats.name <> 'Blair' AND cats.id IN (
              ...
              FROM cattoys 
              INNER JOIN toys ON cattoys.toy_id = toys.id
  ...
  execute(<<-SQL)
    ...
    WHERE
      toys.id IN (
              ...
              FROM cattoys 
              INNER JOIN cats ON cattoys.cat_id = cats.id
...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
    SELECT
      toys_shiny.name, toys_shiny.price
    FROM
      toys JOIN
      toys toys_shiny ON toys.price = toys_shiny.price AND toys_shiny.name <> 'Shiny Mouse' JOIN 
      cattoys ON toys_shiny.id = cattoys.id JOIN 
...
def just_like_orange
  ...
  execute(<<-SQL)
    ...
    FROM
      cats AS double 
    JOIN
      cats ON double.breed = cats.breed
    WHERE 
      cats.name <> 'Orange' AND double.name = 'Orange'
    ORDER BY
      cats.name ASC
  SQL
end
